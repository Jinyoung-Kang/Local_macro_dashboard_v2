...
## 3. 화면 구성 (16개 메뉴 — 구버전 12개 + 분석 4개)

| 메뉴 | 경로 | 내용 |
|---|---|---|
| 📊 거시경제 매크로 지표 | `/macro` | **📋 전체 대시보드 원본 데이터 보기/복사**(상단), **자동 갱신 간격 선택**(10초~30분·끄기 — 4-12), 환율·국채·원자재·지수, 10Y−2Y·30Y−2Y 금리차(공식 확정치 + 스크래핑 시세 병기), 신용·변동성 리스크, **심화 지표 6종** (차트 기간 1~10년 선택), **💱 환율·달러인덱스 비교**(달러/엔·원/달러·엔/원·DXY를 골라 겹쳐 보기 — 기준일 100 또는 원래 단위) |
| 🏢 연준 순유동성 트래커 | `/liquidity` | WALCL − TGA − ON RRP, 구성 항목 분해(자릿수가 달라 패널·단위를 분리), 4주/12주 모멘텀 · 금액은 **달러 + 원화 환산** 병기 |
| 🔄 섹터 & 자산군 로테이션 | `/sector` | S&P 11개 섹터 + 자산군 수익률·순위·벤치마크 대비 초과성과 |
| 📑 기관 13F 포트폴리오 | `/institutions` | 기관별 분기 보유 종목, 분기 대비 액션, **비중 히트맵**(분기 × 종목) + 비중 추이 · 총액은 **달러 + 원화 환산** 병기 |
| 🎯 기관 13F Money 교집합 | `/consensus` | 여러 기관 공통 보유·동시 매수/매도 집중도, **이번 분기 공통 신규 매수** |
| 🧬 구루 포트폴리오 분석 | `/guru` | 기관별 집중도(유효 종목 수)·회전율, **기관 간 유사도 행렬**, 포트폴리오 위험(변동성·VaR·베타·추적오차·종목별 기여), 역방향 조회 |
| 🩺 종목 스코어카드 | `/scorecard` | 모멘텀·변동성·낙폭·추세를 대형주 유니버스 대비 **백분위**로. **재무·성장·밸류에이션은 없습니다**(수집하지 않음) |
...
| 📡 외국인/기관 수급 레이더 | `/radar` | 투자자별 순매수 상위, **🤝 외국인·기관 공통 순매수/순매도**(두 상위 N 목록의 교집합), 소스 진단, 누적 이력 · **받을 수 없는 투자주체는 '지원 안 함'으로 표시**(6-1) |
...
### 4-9-2. 단위가 다른 계열을 겹칠 때는 기준일을 100으로 맞춥니다
...
### 4-10-3. 위험 기여의 합은 포트폴리오 변동성과 같습니다
...
| `slow` | `fred_series` · `fed_liquidity` · `krx_futures` · `sector_history` · `fx_history` · `equity_history` · `volatility_history` · `cot_history` · `daum_futures_trend` | 1시간 |
| `weekly` | `sec_13f` | 12시간 |
