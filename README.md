...
17개 메뉴입니다. 굵은 항목은 이 대시보드에만 있는 것입니다.
...
| 🏠 서울 아파트 실거래 | `/housing` | **25개 구 월별 거래량·평당가 중위값(전 거래 합산), 구별 전년 동월 대비**, 잠정치 표시 (국토부) |
...
| 🇰🇷 국내 파생 & 투기세력 | `/krx` | KOSPI200 선물 OI·베이시스·4대 국면·한국판 COT Index, **증시 규모(시가총액·거래대금, 금융위 공식)** |
| 📡 외국인/기관 수급 레이더 | `/radar` | 투자자별 순매수 상위, **외국인·기관 공통 순매수/순매도**, **순매수 종목의 재무·밸류에이션(DART 부채비율·증가율 + PER·PBR)**, 소스 진단 |
| 🧭 시장 국면 판정 | `/regime` | 성장·신용 축 × 유동성 축으로 4국면 + 근거 지표 |
| 🔗 지표 상관관계 | `/correlation` | 지표 20종 중 둘을 골라 롤링 상관계수·산점도 |
| 🗄️ 데이터 저장소 상태 | `/status` | 태스크별 수집 결과·실패 사유·수동 재실행, **국내 공공 API 연결 진단** |
...
| `DATA_GO_KR_SERVICE_KEY` | 공공데이터포털 **일반 인증키 하나**로 특일정보·주식시세·실거래가를 씁니다(서비스별 활용신청 필요). 없으면 시계는 내장 휴일 표(2025~2028)로, 공식 시세·실거래 화면은 비어 있음 |
...
| 서울 아파트 실거래 | 국토교통부 실거래가 | 공식 (신고 기한 30일 → **최근 2개월 잠정**) |
...
| 영역 | 채택 | 왜 |
|---|---|---|
| Frontend | TypeScript · React 18 · Next.js 15 · Tailwind · Recharts | 17개 메뉴가 서로 다른 표·차트를 쓰므로 컴포넌트 재사용이 크게 이득 |
