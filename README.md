...
| `AI_TIMEOUT_SECONDS` | AI 응답 대기 한도(기본 240초). 추론형 모델이 자주 끊기면 늘리세요 |
...
요청만 해 두고(비동기) 화면에는 기존 값을 즉시 보여 줍니다. 신선도 배지가
"언제 수집한 값인지"를 이미 말해 주고, 수집이 끝나면 다음 조회에서 새 값이
나옵니다. 예전에는 수집이 끝날 때까지 붙잡고 있어서 화면 한 번 여는 데
...
### 4-9. 과거 조회가 안 되는 소스는 우리가 이력을 쌓습니다
